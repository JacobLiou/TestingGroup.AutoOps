...
public sealed class StationCapabilityComplianceChecker
{
    ...
    private StationCapabilityComplianceResult CheckCore(RunbookStepDefinition? step, DiagnosticRunContext? runContext)
    {
        ...
        if (req is null)
        {
            return new StationCapabilityComplianceResult
            {
                ...
                FailReasons = ["工位能力要求不可用"]
            };
        }

        var (actual, source) = LoadActual(runContext);
        if (step is not null && IsCustomRule(step))
        {
        ...
        string source)
    {
        ...
        if (!TryResolveNumericMetric(metricKey, req, actual, out var metricName, out var actualValue, out var defaultOperator, out var defaultThreshold))
        {
            return new StationCapabilityComplianceResult
            {
                ...
                FailReasons = [$"不支持的 metricKey: {metricKey}"]
            };
        }
        ...
        if (!pass)
        {
            ...
            else
            {
                failReasons.Add(template
                    .Replace("{metric}", displayName, StringComparison.OrdinalIgnoreCase)
                    .Replace("{actual}", actualLabel, StringComparison.OrdinalIgnoreCase)
                    .Replace("{required}", required, StringComparison.OrdinalIgnoreCase));
            }
        }
    ...
    private static string FormatMetric(string metricKey, double value)
    {
        ...
        {
            "grr" or "grrpercent" or "gds" or "gdspercent" => $"{value:F2}%",
            "maxoutputpower" or "maxoutputpowerdbm" => $"{value:F2} dBm",
            "snr" or "snrdb" => $"{value:F2} dB",
            "switchrepeatability" or "switchrepeatabilitydb" => $"{value:F2} dB",
            "powerstability" or "powerstabilitydb" => $"{value:F2} dB",
            _ => value.ToString("F2")
        };
    }
