...
public sealed class DeviceVersionComplianceChecker
{
    ...
    {
        public List<ActualDeviceItem> Devices { get; set; } = [];
    }
    ...
    private static string ResolveRequirementUrl(string tmsHealthUrl, RunbookStepDefinition step)
    {
        ...
        return tmsHealthUrl.Replace("/health", "/version-requirements", StringComparison.OrdinalIgnoreCase);
    }
    ...
    private static List<DeviceVersionRequirement> ParseRequirements(string responseBody)
    {
        ...
        if (root.ValueKind == JsonValueKind.Object)
        {
            ...
            {
                requirements.AddRange(ParseRequirementArray(devices));
            }
            else if (root.TryGetProperty("requirements", out var reqs) && reqs.ValueKind == JsonValueKind.Array)
            {
                requirements.AddRange(ParseRequirementArray(reqs));
            }
            else if (root.TryGetProperty("items", out var items) && items.ValueKind == JsonValueKind.Array)
            {
                requirements.AddRange(ParseRequirementArray(items));
            }
        }
        else if (root.ValueKind == JsonValueKind.Array)
        {
            requirements.AddRange(ParseRequirementArray(root));
        }
    ...
    private static List<DeviceVersionRequirement> ParseRequirementArray(JsonElement array)
    {
    ...
    private static List<DeviceVersionActual> LoadActualVersions()
    {
        try
        {
            var path = Path.Combine(AppContext.BaseDirectory, ActualVersionsConfigRelativePath);
            if (!File.Exists(path))
            {
                return [];
            }
            ...
            var config = JsonSerializer.Deserialize<ActualVersionConfig>(json);
