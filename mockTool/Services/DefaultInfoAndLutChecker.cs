...
public sealed class DefaultInfoAndLutChecker
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // Check 1: default info completeness and accuracy
        try
        {
        ...
        // Check 2: LUT download integrity
        try
        {
            using var response = await HttpClient.GetAsync(lutDownloadUrl, cancellationToken);
            var lutContent = await response.Content.ReadAsStringAsync(cancellationToken);
            ...
            {
                var hasHeader = lutContent.Contains("LUT_VERSION", StringComparison.OrdinalIgnoreCase);
                var hasData = lutContent.Contains("TABLE_BEGIN", StringComparison.OrdinalIgnoreCase) &&
                              lutContent.Contains("TABLE_END", StringComparison.OrdinalIgnoreCase);
    ...
    private static string ResolveDefaultInfoUrl(RunbookStepDefinition step, string mimsXml, string tmsHealthUrl)
    {
        ...
        return tmsHealthUrl.Replace("/health", "/default-info", StringComparison.OrdinalIgnoreCase);
    }
    ...
    private static string ResolveLutDownloadUrl(RunbookStepDefinition step, string mimsXml, string tmsHealthUrl)
    {
        ...
        return tmsHealthUrl.Replace("/health", "/lut/download/default", StringComparison.OrdinalIgnoreCase);
    }
